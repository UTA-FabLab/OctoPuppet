...
class LocalFileStorage(StorageInterface):
    ...
    def _get_source_destination_data(self, source, destination, must_not_equal=False):
        ...
        if (
            os.path.exists(destination_fullpath) and
            source_fullpath != destination_fullpath
        ):
            raise StorageError(
    ...
    def _set_display_metadata(self, destination_data, source_data=None):
        if (
            source_data and
            destination_data["name"] == source_data["name"] and
            source_data["name"] != source_data["display"]
        ):
    ...
    def add_file(
        ...
        try:
            gcodeIdent = str(file_object.stream().readline().strip())
            if 'generated by PrusaSlicer' in gcodeIdent:
                gcodePrusa = True
            elif 'PolyPrinter post-processing' in gcodeIdent:
                gcodePoly = True
            else:
                print("Invalid GCODE flavor")
            ...
            print("Error when identifying GCODE type")
            raise e
        ...
        if build_time is not None:
            if gcodePoly:
                re1 = r'.*?'  # Non-greedy match on filler
                re2 = r'([+-]?\d*\.\d+)(?![-+0-9\.])'  # Float 1

                rg = re.compile(re1 + re2, re.IGNORECASE | re.DOTALL)
                r = rg.search(build_time)
                if r:
                    secs = float(r.group(1)) * 60
                    m, s = divmod(secs, 60)
                    h, m = divmod(m, 60)

            if gcodePrusa:
                re1 = r'.*?'  # Non-greedy match on filler
                re2 = r'((?:\d*)h) ((?:[0-5]\d*|\d)m) ((?:[0-5]\d*|\d)s)'  # Time string : 'Hh Mm Ss'

                rg = re.compile(re1 + re2, re.IGNORECASE)
                r = rg.search(build_time)
                if r:
                    h = r.group(1).strip('h')
                    m = r.group(2).strip('m')
            ...
            build_time_str = str(int(h)).zfill(2) + ":" + \
                            str(int(m)).zfill(2) + ":" + \
                            str(int(s)).zfill(2)
        else:
        ...
        if build_mat_used is not None:
            if gcodePrusa:
                ...
                rg = re.compile(re1 + re2, re.IGNORECASE | re.DOTALL)
                r = rg.search(build_mat_used)
                build_mat_used_str = r.group(1)

            if gcodePoly:
                re1 = r'.*?'  # Non-greedy match on filler
                re2 = r'([+-]?\d*\.\d+)(?![-+0-9\.])'  # Float 1
                re3 = r'.*?'  # Non-greedy match on filler
                re4 = r'([+-]?\d*\.\d+)(?![-+0-9\.])'  # Float 2

                rg = re.compile(re1 + re2 + re3 + re4, re.IGNORECASE | re.DOTALL)
                r = rg.search(build_mat_used)
                build_mat_used_str = r.group(2)
        else:
            print("WARNING - Missing material used data. Setting to 0")
            build_mat_used_str = 0

        metadata["est_flmnt_vol"] = build_mat_used_str
        metadata_dirty = True
